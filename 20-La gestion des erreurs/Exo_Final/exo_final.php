...
function processForm()
{
  ...
  if (!$user->isfull()) {
    throw new InvalidArgumentException('Une erreur est survenue : l\'utilisateur est incomplet');
  }
 ...
 * 
 * Exception lorsqu'un type de contenue n'est pas valide
 */
 ...
 * 
 * Exception lorsque la date n'est pas valide 
 */
...
class User
{
  ...
  public function setName(string $name)
  {
    ...
    if (!ctype_alpha($name)) {
      throw new InvalidContentTypeException("LA proprété name ne peut contenir que des lettres");
    }
    $this->name = $name;
  }
  /**
  ...
  public function setBirthDate(string $birthDate)
  {
    if (!date_create($birthDate)) {
      throw new InvalidDateTimeException('La date de naissance n\'a pas pu être identifié');
    }
    ...
    if ($birthDate > $now) {
      throw new InvalidDateTimeException('la date ne peut pas se situer dans le futur');
    }
  ...
  public function setPassword(string $password)
  {
    ...
    if (strlen($password) < self::PASSWORD_MIN_LENGTH) {
      throw new LengthException("La proprété passowrd ne peux excéder " . self::PASSWORD_MIN_LENGTH . " caractères");
    }
    if (!ctype_alnum($password)) {
      throw new InvalidContentTypeException("La propriété password ne peut contenir que des lettres et des chiffres");
    }
  ...
  public function setComment(?string $comment)
  {
    ...
    if (strlen($comment) > self::COMMENT_MAX_LENGTH) {
      throw new LengthException("La proprété comment ne peut contneir que " . self::COMMENT_MAX_LENGTH . " caractères");
    }
